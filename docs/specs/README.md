...
| --- | --- | --- | --- |
| `cross-contract-conformance.md` | Mechanical parity across transport/OpenAPI/generated clients, API fakes/real stores, and tenant-reference owners (iter-0183, D-0251) | revision 1; seven acceptance invariants | IMPLEMENTED at iter-0183; lifecycle remains open for owner closure, no product behaviour change |
| `ops-audit-log-retention.md` | Bounded instance-wide retention for organization and global audit rows (FR-033/NFR-027; planned iter-0184) | revision 1 DRAFT; strict config, fenced bounded purge, observability and recovery contract | NOT IMPLEMENTED; iter-0184 is commissioned but not opened |
| `func-project-gate-policy.md` | One project reliability-gate policy inherited by services without an explicit override (FR-034/NFR-028; planned iter-0185) | revision 1 DRAFT; effective source tuple, CAS, override revocation, API/UI and tenancy contract | NOT IMPLEMENTED; depends on owner approval and an approved SPA mock |
| `func-reliability-gate-all-windows.md` | Worst-of-all-configured-windows gate evaluation without averaging healthy and unhealthy evidence (FR-035/NFR-029; planned iter-0186) | revision 1 DRAFT; policy schema v2, aggregation algebra, evidence and compatibility contract | NOT IMPLEMENTED; depends on iter-0185 and explicitly supersedes FR-024 D2 only after approval |
| `func-onboarding.md` | The first truthful path from an authorized account to one real monitor result (FR-036/NFR-030; planned iter-0187) | revision 1 DESIGN BRIEF; design questions, non-negotiable constraints and approval checklist | NOT DESIGNED/NOT IMPLEMENTED; iter-0187 is design-only and production work requires a separately approved mock |
...
| `func-truthful-rendering.md` | A rendering claims no more than the facts behind it (FR-031/NFR-025, opened after iter-0173): the reliability timeline's geometry and five encodings, the monitor's Response time panel, and the /sla project-objective card | full (28 invariants, §10) — design approved, D-0235 | IMPLEMENTED at iter-0174 (2026-09-03): the owner-approved mock, then the three surfaces; no Go code and no API change. NFR-025c is still open |
| `func-async-canary.md` | A typed external canary for an async API journey — submit → poll → verify as ONE transaction (FR-029/NFR-024) | full (14 invariants, §6) — approved at revision 8, after six review rounds and seventeen P0s | IMPLEMENTED: phases A–F (iter-0169, iter-0171); both former residuals closed at iter-0171 (D-0231) and iter-0172 (D-0233), so no FR-029 row is `PARTIAL` |
| `sec-synthetic-secrets.md` | A credential inside a synthetic scenario is a secret everywhere: no URL in a result, the scenario encrypted at rest, declared bindings resolved from the project inventory (FR-028/NFR-023) | full (16 invariants, §6) — approved at revision 3, amended by the owner's §10 ruling | IMPLEMENTED: all three stages at iter-0166 (D-0217); the stage-2 binding editor landed at iter-0167 |
| `func-monitor-description.md` | A monitor says what it is for: one optional 200-character description, shown where monitors are listed, not matched by search (FR-030) | full (7 invariants, §6) — mock approved 2026-09-03, D-0234 | IMPLEMENTED and closed at iter-0173 |
| `func-incident-audit.md` | An incident write says who wrote it (FR-026/NFR-021): only PRINCIPAL writes, the row written inside the mutating transaction, read through the organization's existing audit listing | full (22 invariants, §6), revision 4 | IMPLEMENTED at iter-0168 (D-0219) |
| `func-service-escalation.md` | A Service escalates its own outage (FR-023/NFR-018) | full (16 invariants, §6) | DELIVERED at iter-0157; §8's non-goals still bind — retroactive escalation was found contradicted and fixed at iter-0161 §13 |
