...
| `func-reliability-gate.md` | A deploy asks whether the error budget allows it (FR-024/NFR-019, opened after iter-0161) | full (21 invariants, §7 matrix); design approved at revision 13 (D-0201) | IN IMPLEMENTATION (iter-0163, resumed by D-0204): schema, domain, config, metrics and CLI in the tree; routes, store and the §7 matrix in progress; the SPA waits on the owner's visual pass on the approved mock |
