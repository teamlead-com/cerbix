...
delegation begins suppressing the member's own alert. Withheld onsets are counted as `cerbix_service_alert_withheld_total{signal,reason}`, the reason being `unroutable` or `no_governing_revision`.
...
| 14 | `onset_undelivered` | The announcement WAS made and reached nobody: the worker resolved zero recipients and stopped, because no retry reaches a channel that has been deleted, and the latch stays firing so no further edge is coming. Coverage means somebody was told (D-0179). | Fix or replace the channel. Coverage returns at the next announcement that is actually received. |
| 15 | `latch_inconsistent` | Burn only: an eligible latch is in a state the evaluator does not write (`clear` while still marked firing), so the row cannot be read as either a live announcement or a finished one. Also the classifier's last case — if the conjunction refused and nothing above explains it, the latch is uninterpretable by construction (D-0178). | Report it. This is a defect or legacy state, never a configuration somebody chose. |
