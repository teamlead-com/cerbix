...
| NFR-014 | Monitoring as Code reconciliation is tenant-scoped, atomic per project bundle, HA-safe, bounded, no-op idempotent, last-known-good preserving, and never auto-deletes history or accepts inline secrets. | IN_PROGRESS | Strict/bounded parsing + scope matrix + inline-secret rejection + canonical no-op detection delivered pure (iter-0087, [`internal/fileprovider/`](../internal/fileprovider/)); atomic transactional apply, HA leadership, and LKG/orphan runtime pending (iter-0088+). Spec §§2, 7–18; D-0145. |
