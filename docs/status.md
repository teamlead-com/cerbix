...
| NFR-014 | Monitoring as Code reconciliation is tenant-scoped, atomic per project bundle, HA-safe, bounded, no-op idempotent, last-known-good preserving, and never auto-deletes history or accepts inline secrets. | DONE | Delivered across iter-0087…0094: strict/bounded YAML+config parsing, scope matrix, inline-secret rejection, canonical no-op idempotency, per-project atomic apply (no partial/silent fallback), HA-safe single active reconciler per provider (advisory leadership), D-0142 revision-safe updates, last-known-good on invalid input, orphan/disable with no history loss, no inline secrets/hard delete/unbounded labels. Verified by unit + real-Postgres integration (both storage modes) + real-binary no-restart E2E. Spec §§2, 7–18; D-0145/D-0146. |
