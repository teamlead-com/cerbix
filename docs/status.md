...
| FR-017 | Monitoring as Code: tenant-scoped file providers hot-reconcile versioned ProjectBundles into provider-owned monitors without service restart/reload; UI/API monitors coexist. | TODO | Contract only: [`func-monitoring-as-code.md`](specs/func-monitoring-as-code.md), D-0145. No runtime implementation yet. |
...
| NFR-014 | Monitoring as Code reconciliation is tenant-scoped, atomic per project bundle, HA-safe, bounded, no-op idempotent, last-known-good preserving, and never auto-deletes history or accepts inline secrets. | TODO | Contract only: [`func-monitoring-as-code.md`](specs/func-monitoring-as-code.md) §§2, 7–18; D-0145. |
