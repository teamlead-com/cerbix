...
| FR-017 | Monitoring as Code: tenant-scoped file providers hot-reconcile versioned ProjectBundles into provider-owned monitors without service restart/reload; UI/API monitors coexist. | TODO | Contract only: [`func-monitoring-as-code.md`](specs/func-monitoring-as-code.md), D-0143. No runtime implementation yet. |
...
| NFR-010 | Local-login passwords hashed (argon2id), never logged, rate-limited. | DONE | argon2id + uniform 401 + no-secret-logging ([`internal/auth/password.go`](../backend/internal/auth/password.go), [`local.go`](../backend/internal/auth/local.go)); **per-IP sliding-window rate limit** ([`internal/auth/ratelimit.go`](../backend/internal/auth/ratelimit.go), `login_rate_limit_per_minute` default 10, 429 over limit) — D-0031. |
...
| NFR-014 | Monitoring as Code reconciliation is tenant-scoped, atomic per project bundle, HA-safe, bounded, no-op idempotent, last-known-good preserving, and never auto-deletes history or accepts inline secrets. | TODO | Contract only: [`func-monitoring-as-code.md`](specs/func-monitoring-as-code.md) §§2, 7–18; D-0143. |
