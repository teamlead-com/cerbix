...
export interface paths {
    ...
    "/api/v1/projects/{projectID}/monitors/{monitorID}/expected-runs": {
         ...
         *
         *     The range is half-open `[from, to)`, both REQUIRED, `from < to`, at most the retention window, whose DEFAULT is 14 days and which an instance may configure through `ledger.expected_run_retention_days` — the bound this endpoint enforces is the instance's, not this number. The figure here is checked against `domain.DefaultExpectedRunRetentionDays` by `make docs-check`: a static document cannot read a runtime value, so it is not derived, and the two are held equal instead. Order is `due_at DESC`; no tiebreak is needed and the reason is structural — the route is monitor-scoped and the primary key is `(monitor_id, due_at)`, so `due_at` is unique within one monitor. `cursor` is an opaque keyset of the LAST RETURNED item, the next page is bound strictly below it, and `next_cursor` is null on the last page. A cursor outside `[from, to)` is 400 rather than ignored: it cannot belong to this traversal, and ignoring it would return a page from a different query than the caller asked for.
         *
...
export interface components {
    schemas: {
        ...
        ServiceSignalState: {
             ...
             * @enum {string}
             */
            reason?: "not_owned" | "policy_pages_nothing" | "state_not_pageable" | "never_evaluated" | "generation_changed" | "revision_changed" | "evaluation_error" | "stale_lease" | "no_enabled_target" | "held" | "rule_unevaluated" | "unroutable" | "onset_pending" | "onset_undelivered" | "latch_inconsistent";
            /**
             * Format: date-time
        ...
        ServiceDetail: {
            ...
            /** @description ALWAYS NULL, and it stays that way by design — not because the numbers are missing. SLO, error budget and burn rate live on `GET …/services/{serviceID}/reliability`, which carries them together with the honesty they require: the window's status and reason, storage continuity, decidable coverage, per-revision segments and the seal watermark they were computed from. A bare number on THIS object would have none of that context, and a zero a client could render as a number is exactly the confident falsehood this feature exists to prevent. (Phase 2 shipped in iter-0144; this field predates it and is kept so the shape of the detail response does not change.) */
            reliability: unknown;
        ...
        Monitor: {
            ...
            region?: string;
            /** @description Per-type config; for composite: {children:"id1,id2", mode:"all|any"}; for promql: {query, and with auth_mode:"basic" also username + password|password_ref}; for synthetic: {scenario} plus one scenario_secret_<binding>_ref per credential, naming a project secret the scenario references as {{secret:<binding>}} — synthetic only, and refused on any other type. A credential-bearing header (authorization, cookie, x-api-key, ...) must hold exactly the placeholder, no placeholder may resolve into a URL, and a scenario carrying bindings is not testable before it is saved. For async_canary: {workflow} — ONE JSON document of kind async_transaction_v1, plus one canary_secret_<binding>_ref per binding, naming a project secret the document references as {"secret_ref":"<binding>"}; async_canary only, and refused on any other type. The document is nested and typed with closed unions and no free-form field: submit (http_json|multipart_fixture, POST only, 2xx accepted_status, its own submit_timeout <= the monitor timeout), correlate (response_json path | response_header name), completion (sse|poll_json, timeout <= the monitor timeout), result (max_latency, required_json_fields, lifecycle_path) and cleanup (lifecycle_prefix|none, where none requires acknowledged:true). {{ correlation_id }} is legal ONLY in completion.url and no other placeholder is legal anywhere; a credential-bearing header carries a secret_ref and never a value; a completion header may carry a binding only when the completion host equals the submit host. The server canonicalizes the document on write, so the stored form is byte-identical to the one a Monitoring-as-Code bundle produces. */
            config?: {
                [key: string]: string;
            };
            enabled?: boolean;
