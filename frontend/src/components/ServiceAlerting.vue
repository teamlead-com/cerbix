...
const props = defineProps<{
  ...
  /** What that declaration is currently producing, per signal. */
...
function badge(signal?: SignalState | null): { label: string; tone: string; detail: string } {
  if (!signal) return { label: "unknown", tone: "ink-3", detail: "coverage could not be read" };
  if (signal.armed) return { label: "armed", tone: "ok", detail: "members' alerts are delegated" };
  // NOT armed is the safe state, not an error state: the members are paging for themselves. The
  // two tones separate "it will arm by itself" from "somebody has to do something".
  const transient = ["never_evaluated", "generation_changed", "revision_changed", "stale_lease", "held", "rule_unevaluated"];
  const tone = transient.includes(signal.reason ?? "") ? "warn" : "ink-3";
  return {
    label: transient.includes(signal.reason ?? "") ? "pending" : "degraded",
    tone,
    detail: REASONS[signal.reason ?? ""] ?? signal.reason ?? "not armed",
  };
}

const live = computed(() => badge(props.state?.live));
const burn = computed(() => badge(props.state?.burn));
  ...
  <section class="mb-4 overflow-hidden rounded border border-border bg-surface shadow-card" data-testid="service-alerting">
    <header class="flex flex-wrap items-center gap-2 border-b border-border px-4 py-[10px]">
      ...
      <span
        ...
        :class="{
          'bg-ok/10 text-ok': sig.b.tone === 'ok',
          'bg-warn/10 text-warn': sig.b.tone === 'warn',
          'bg-ink-3/10 text-ink-3': sig.b.tone === 'ink-3',
        }"
    ...
    <div v-else class="space-y-3 px-4 py-3 text-[12.5px]">
      <!-- The coverage sentence, which is the whole point of separating it from the switch. -->
      <p class="text-ink-2" data-testid="alerting-coverage">
        <template v-if="live.label === 'armed' || burn.label === 'armed'">
          Members' alerts are delegated where coverage is armed; everywhere else they page for
          themselves.
        </template>
        <template v-else>
          Members page for themselves. Live: {{ live.detail }}. Burn: {{ burn.detail }}.
        </template>
      </p>
