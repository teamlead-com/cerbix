...
// G3 — the ledger's client-visible bounds are PUBLISHED, in the shape the monitor and canary bounds
// already use, so the SPA stops re-typing them as literals.
//
// `MonitorDetailView.vue` carried `expectedRunRetentionDays = 14` and `expectedRunMinRetentionDays
// = 2` as its own constants. Neither is a client decision: the first is the server's default and
// the second is the bound the API enforces, so a change to either left the view asking for a range
// the server refuses — the drift the canary and monitor bounds fixtures exist to make impossible.
//
// The fixture is WRITTEN by this test and READ by `frontend/src/lib/expectedRunBounds.spec.ts`, so
// a value changed here and not there fails on the client side.
func TestExpectedRunBoundsArePublishedForTheClient(t *testing.T) {
	out, err := json.MarshalIndent(map[string]any{
		"note": "GENERATED by internal/domain/expectedrun_test.go. " +
			"frontend/src/lib/expectedRunBounds.spec.ts asserts the client mirrors each value.",
		"bounds": map[string]int{
			"EXPECTED_RUN_RETENTION_DEFAULT_DAYS": DefaultExpectedRunRetentionDays,
			"EXPECTED_RUN_RETENTION_MIN_DAYS":     MinExpectedRunRetentionDays,
			"EXPECTED_RUN_RETENTION_MAX_DAYS":     MaxExpectedRunRetentionDays,
		},
	}, "", "  ")
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile("testdata/expected_run_bounds.json", append(out, '\n'), 0o600); err != nil {
		t.Fatal(err)
	}
}
