...
func httpMember(id, region string) Member {
	return Member{
		...
		StaleAfter: ResolveStaleAfter(DefaultFreshness(), domain.MonitorHTTP, 30*time.Second, 0),
	}
}
...
func pushMember(id, region string, armed time.Time) Member {
	return Member{
		...
		StaleAfter: ResolveStaleAfter(DefaultFreshness(), domain.MonitorPush, 60*time.Second, 0),
		ArmedAt:    armed,
	}
}

func allPolicies() Policies {
	return Defaults(Policies{MaintenanceExcludes: true}, map[string]int{"core": 1}, 1)
}
...
func TestIgnoreDropsAnUnknownMemberWhileOthersDecide(t *testing.T) {
	...
	p := Defaults(Policies{MissingData: MissingIgnore, MaintenanceExcludes: true}, map[string]int{"core": 2}, 1)
...
func TestIgnoreCannotLaunderTheLastUnknownIntoExcluded(t *testing.T) {
	p := Defaults(Policies{MissingData: MissingIgnore, MaintenanceExcludes: true}, map[string]int{"core": 1}, 1)
	b := reduce(t, Input{
		Start: bucketStart, End: bucketEnd,
...
func TestAllMembersExcludedIsExcludedNeverGood(t *testing.T) {
	...
	b := reduce(t, Input{
		...
		Policies: Defaults(Policies{MaintenanceExcludes: true}, map[string]int{"core": 2}, 1),
	})
	if b.Durations.Excluded != time.Minute {
		t.Errorf("excluded = %s, want the whole bucket", b.Durations.Excluded)
	}
...
func TestQuorumClampsToEligibleAndRecordsIt(t *testing.T) {
	...
	p := Defaults(Policies{
		Aggregation:         Aggregation{Mode: AggQuorum, DegradedMin: 3, HealthyMin: 3},
...
func TestOneDarkRegionIsDegradedNotDown(t *testing.T) {
	...
	p := Defaults(Policies{MaintenanceExcludes: true},
...
func TestReduceIsDeterministic(t *testing.T) {
	in := Input{
		...
		Policies:    Defaults(Policies{MaintenanceExcludes: true}, map[string]int{"core": 1, "geo1": 1}, 2),
	}
	first, err := Reduce(in)
	if err != nil {
		t.Fatal(err)
